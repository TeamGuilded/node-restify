...
Router.prototype.find = function find(req, res, callback) {
	...
	// This built-in restify cors plugin is jacked-up. So much so that it was removed in restify v5
	// In short, if the request Origin is not in the list of allowed origins
	// 'Access-Control-Allow-Origin: *'' is returned, allowing all origins (see code below)
	// We just disable the plugin and let restify-cors-middleware handle it
	return (false);
